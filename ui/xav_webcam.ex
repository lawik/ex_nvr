defmodule ExNVR.Pipeline.Source.Webcam do
  ...
  def handle_init(_ctx, %__MODULE__{} = options) do
    ...
    IO.inspect(options, label: "webcam options")
    # r = Xav.Reader.new!(options.device, device?: true, out_format: :yuv420p)
    r = Xav.Reader.new!("/dev/video0", device?: true, out_format: :yuv420p)
  ...
  def handle_setup(_ctx, state) do
    {:ok, frame} = Xav.Reader.next_frame(state.reader)
  ...
  def handle_playing(ctx, state) do
    ...
    {:ok, provider} =
      Membrane.UtilitySupervisor.start_link_child(
        ...
        Supervisor.child_spec({Task, fn -> frame_provider(state.reader, element_pid) end}, [])
      )
  ...
  defp frame_provider(reader, target) do
    case Xav.Reader.next_frame(reader) do
      {:ok, frame} ->
        send(target, {:frame, frame})
        frame_provider(reader, target)
