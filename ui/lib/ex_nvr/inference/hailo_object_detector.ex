if Code.ensure_loaded?(Hailo) do
  defmodule ExNVR.Inference.HailoObjectDetector do
    @moduledoc """
    ...
    @behaviour ExNVR.Inference.InferencePipeline

    use Membrane.Sink

    require Membrane.Logger

    alias ExNVR.Inference.Detection
    ...
    def handle_buffer(:input, buffer, _ctx, state) do
      ...
      detections =
        ...
        |> to_detections(ratio, w_pad, h_pad)
    ...
    defp to_detections(raw_detections, ratio, w_pad, h_pad) do
      Enum.map(raw_detections, fn det ->
        w = det.xmax - det.xmin
        h = det.ymax - det.ymin
        cx = det.xmin + w / 2
        cy = det.ymin + h / 2
    ...
    @impl ExNVR.Inference.InferencePipeline
    ...
    def validate_config(config) do
      ...
      {:ok,
       %{
         ...
         "prob_threshold" =>
           ExNVR.Inference.InferencePipeline.parse_float(config["prob_threshold"], 0.25),
         "only_keyframes" =>
           ExNVR.Inference.InferencePipeline.parse_bool(config["only_keyframes"], true)
       }}
    end
