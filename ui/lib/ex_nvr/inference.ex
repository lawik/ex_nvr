defmodule ExNVR.Inference do
  ...
  # --- Device association (via trigger source configs) ---

  @doc """
  Returns the distinct inference pipelines referenced by object_detection
  trigger source configs for a device.
  """
  @spec inference_pipelines_for_device(binary()) :: [Pipeline.t()]
  def inference_pipelines_for_device(device_id) do
    device_id
    |> ExNVR.Triggers.trigger_configs_for_device()
    |> Enum.flat_map(& &1.source_configs)
    |> Enum.filter(&(&1.source_type == "object_detection"))
    |> Enum.map(& &1.config["inference_pipeline_id"])
    |> Enum.reject(&is_nil/1)
    |> Enum.uniq()
    |> Enum.map(&get_pipeline/1)
    |> Enum.reject(&is_nil/1)
  end
end
