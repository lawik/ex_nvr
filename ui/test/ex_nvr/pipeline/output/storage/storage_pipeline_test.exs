defmodule ExNVR.Pipeline.Output.Storage.StoragePipelineTest do
  ...
  describe "terminate request" do
    test "flushes the in-flight segment to disk and database", %{device: device} do
      ...
      # feed only a part of the stream (not enough to complete a segment)
      # and do not send end of stream
      actions =
  ...
  describe "timestamp correction" do
    test "small drift is clamped to ±30ms", %{device: device} do
      ...
      # the second segment end date is adjusted towards the wall clock
      # by at most 30 ms (the last segment is flushed by end of stream
  ...
  describe "database failure" do
    ...
    test "segments stay on disk and the element keeps running", %{device: device} do
      # See issue #40: when the database insert fails the run/recording
      # is dropped and the segment is orphaned on disk. This test pins
  ...
  defp perform_discontinuity_test(device, event) do
    ...
    # the second event hits the element with no open segment and is a no-op
    actions =
